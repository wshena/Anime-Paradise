...
const page = () => {
  ...
  useEffect(() => {
    async function fectData(page:number) {
      const apiData = await axios.get(`${process.env.NEXT_PUBLIC_HOST_API}/top/manga?page=${page}`);
      const pagination = apiData.data.pagination;
      const manga = apiData.data.data
      if (manga) {
        setManga(manga)
      }
  ...
  return (
    <section className="w-full">
      <div className="container">
        <div className="pb-7">
          <h1 className="font-bold text-[2rem]">Top Manga</h1>
        </div>
