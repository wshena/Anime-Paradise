...
const page = () => {
    ...
    async function fectData(page:number) {
      const apiData = await axios.get(`${process.env.NEXT_PUBLIC_HOST_API}/recommendations/anime?page=${page}`);
      const pagination = apiData.data.pagination;
      const animeRecomendations = apiData.data.data
      if (animeRecomendations) {
        setRecomend(animeRecomendations)
      }
    ...
    <section className="w-full">
      <div className="container">
        <div className="pb-5 md:pb-7">
          <h1 className="font-bold text-[2rem]">Anime Recomendations</h1>
        </div>
										...
										{item.entry.map((anime) => {
											return (
												<div className="p-2 lg:p-6 border-4 rounded-[10px]">
													<img src={anime.images.jpg.image_url} className='w-[100px] h-[150px] md:w-[150px] md:h-[200px] lg:w-[200px] lg:h-[200px]'/>
													<h1 className='mt-3 font-bold w-[100px] md:w-[150px] lg:w-[200px] text-[0.6rem] lg:text-[1.2rem]'>{anime.title}</h1>
												</div>
											)
										})}
									</div>
